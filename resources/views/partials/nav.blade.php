<header class="hidden md:flex sticky top-4 z-40 w-full justify-center">
    <div
        class="flex items-center gap-10 px-8 py-3 rounded-full border border-slate-200/80 bg-white/70 text-slate-800 shadow-sm backdrop-blur transition dark:border-neutral-700/60 dark:bg-black/10 dark:text-slate-50">

        {{-- Brand --}}
        <a href="/" class="flex items-center gap-3">
            <span class="text-lg font-semibold tracking-tight text-slate-800 dark:text-slate-50">
                Rent-Tel
            </span>
        </a>
        ...
        <nav class="hidden md:flex items-center gap-6">
            ...
            <a href="{{ url('/#benefits') }}"
                class="text-sm font-medium text-slate-700 transition hover:text-indigo-500 dark:text-slate-200 dark:hover:text-indigo-400">
                Benefits
            </a>
        ...
        <div class="hidden md:flex items-center gap-3">
            {{-- <button type="button" data-theme-toggle aria-pressed="false"
                class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-300/80 bg-white/70 text-slate-700 shadow-sm transition hover:border-indigo-400 hover:text-indigo-500 dark:border-slate-600/70 dark:bg-white/10 dark:text-slate-200 dark:hover:border-indigo-400 dark:hover:text-indigo-200"
            ...
            @guest
                <a href="{{ route('login') }}"
                    class="rounded-full px-3 py-1.5 text-xs font-semibold text-slate-700 border border-slate-200 hover:border-indigo-500 hover:text-indigo-600 transition dark:text-slate-200 dark:border-slate-600/70 dark:hover:text-indigo-300">
                    Sign in
                </a>
            @endguest
            ...
            @auth
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="rounded-full px-3 py-1.5 text-xs font-semibold text-slate-700 border border-slate-200 hover:border-rose-500 hover:text-rose-500 transition dark:text-slate-200 dark:border-slate-600/70 dark:hover:text-rose-300">
                        Logout
                    </button>
                </form>
            @endauth
...
<div id="mobileMenu" class="md:hidden hidden border-t border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950">
    <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6">
        <nav class="grid gap-2">
            ...
            <div class="mt-2 flex items-center justify-between rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 dark:border-gray-800 dark:text-gray-200">
                <span>Theme</span>
                <button type="button" data-theme-toggle aria-pressed="false"
                    class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-300/80 bg-white/70 text-slate-700 shadow-sm transition hover:border-indigo-400 hover:text-indigo-500 dark:border-slate-600/70 dark:bg-white/10 dark:text-slate-200 dark:hover:border-indigo-400 dark:hover:text-indigo-200"
                    aria-label="Toggle theme">
