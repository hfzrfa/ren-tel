    ...
    <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16">
        <header class="mb-8 flex items-end justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600 hover:text-black">Browse our full catalog and pick your perfect ride.
                </p>
            </div>
            <a href="{{ route('landing') }}"
                class="hidden md:inline-flex items-center gap-1 rounded-full bg-white/80 px-4 py-2 text-sm font-semibold text-black shadow-sm hover:bg-white focus-visible:outline-none focus-visible:ring-2  dark:hover:text-white dark:hover:bg-black">
        ...
        <div class="mt-6 flex items-center gap-2 overflow-x-auto scrollbar-hidden">
            <a href="{{ route('cars.index', $allQuery) }}"
                class="shrink-0 whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold shadow-sm ring-1 ring-black/5 transition-colors {{ $activeType === '' ? 'bg-[#1e1e1e] text-white' : 'bg-white/80 text-black hover:bg-white' }}">
                All
            </a>

            @foreach (($types ?? []) as $type)
                <a href="{{ route('cars.index', array_merge($baseQuery, ['type' => $type])) }}"
                    class="shrink-0 whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold shadow-sm ring-1 ring-black/5 transition-colors {{ $activeType === $type ? 'bg-[#1e1e1e] text-white' : 'bg-white/80 text-black hover:bg-white' }}">
                    {{ strtoupper($type) }}
                </a>
            @endforeach
        </div>
            ...
            @forelse ($cars as $car)
                <article
                    ...
                    <div class="relative aspect-4/3 overflow-hidden">
                        ...
                        <span
                            class="absolute left-3 top-3 inline-flex items-center rounded-full bg-white/90 px-2 py-1 text-[10px] font-medium text-gray-900 ring-1 ring-black/5 backdrop-blur dark:bg-gray-900/80 dark:text-gray-100">{{ strtoupper($car->type) }}</span>
                    </div>
                    <div class="p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <h3 class="line-clamp-1 text-base font-semibold leading-6 text-black">{{ $car->name }}
                                </h3>
                                <p class="mt-0.5 text-black">{{ strtoupper($car->type) }} •
